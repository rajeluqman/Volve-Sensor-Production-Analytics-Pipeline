...
Ported from creative_intelligence_lab's scripts/gen_repo_map.py (pipeline-retrofit effort, via
home-credit's pattern), retargeted to THIS repo's real layout (bronze/, silver/,
databricks_notebooks/, gold/dbt_models/, data_quality/, airflow/dags/, docs/, docs/ADR/)
instead of CIL's models/seeds/dags-at-root layout or home-credit's glue/dbt_home_credit/.
...
EXCLUDE = {
    ".gitignore", ".env.example", "gold/dbt_models/profiles.yml", "gold/profiles.yml",
    ".claude/settings.json", "architecture/REPO_MAP.md",
}
EXCLUDE_PREFIX = (".github/", "docs/screenshots/")
...
ROLE_LABELS = [
    ...
    ("dbt:model", "dbt — other"),
    ("databricks", "Databricks notebooks (Bronze/Silver)"),
    ("bronze", "Bronze ingestion"),
    ("silver", "Silver transforms"),
    ("gold", "Gold KPIs / feature store (no dbt)"),
    ("ml", "ML training + scoring (MLflow)"),
    ("snowflake", "Snowflake serving layer"),
    ("dag", "Airflow DAGs"),
    ("confluence", "Confluence sync pages"),
    ("dq", "Great Expectations / data quality"),
...
def role_of(rel: str) -> str:
    ...
    if rel.startswith("docs/"):
        return "doc"
    if rel.startswith("gold/dbt_models/models/staging/"):
        return "dbt:staging"
    if rel.startswith("gold/dbt_models/models/intermediate/"):
        return "dbt:intermediate"
    if rel.startswith("gold/dbt_models/models/marts/"):
        return "dbt:mart"
    if rel.startswith("gold/dbt_models/tests/"):
        return "dbt:test"
    if rel.startswith("gold/dbt_models/macros/"):
        return "dbt:macro"
    if rel.startswith("gold/dbt_models/"):
        return "dbt:model"
    if rel.startswith("databricks_notebooks/"):
        return "databricks"
    ...
    if rel.startswith("ml/"):
        return "ml"
    if rel.startswith("snowflake/"):
        return "snowflake"
    if rel.startswith("airflow/dags/"):
        return "dag"
    if rel.startswith("airflow/"):
        return "config"
    if rel.startswith("confluence/"):
        return "confluence"
    if rel.startswith("ingestion/"):
        return "other"
