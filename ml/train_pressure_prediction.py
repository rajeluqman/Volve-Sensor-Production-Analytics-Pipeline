#!/usr/bin/env python3
"""
ML Model 1 — Pressure Prediction
Algorithm : XGBoost Regressor
Target    : next_day_pressure (continuous, psi)
Source    : claudecatalog.gold.ml_feature_store
Tracking  : Databricks MLflow  (experiment: /Shared/volve_pressure_prediction)

Split strategy: time-based 80/20 to prevent data leakage across time.
"""
...
MODEL_NAME      = "volve_pressure_prediction"
...
def preprocess(df: pd.DataFrame):
    X = df[FEATURES].copy()
    y = df[TARGET].copy()
    # Lag/rolling features are NULL for the first N rows — impute with column median
...
def run():
    ...
    print(f"Algo  : XGBoost Regressor")
    ...
    with mlflow.start_run(run_name=f"{MODEL_NAME}_xgb"):
